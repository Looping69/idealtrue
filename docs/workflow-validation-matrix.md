...
| Subscription checkout, voucher redemption, card setup, greylisting, webhook completion | Billing, Catalog, Identity, Ops | `/pricing`, `/host`, `/admin` users/billing sections | `billing`: `listPlans`, `createSubscriptionCheckout`, `createHostBillingSetupCheckout`, `listMySubscriptions`, `getMyHostBillingAccount`, `redeemVoucher`, `listAdminHostBilling`, `adminSetHostGreylist`, `getCheckoutStatus`, `yocoWebhook`; `catalog`: greylist visibility restrictions | Host billing source and plan state determine listing entitlements, commercial access, reminders, and greylist locks. |
...
| Booking, enquiry, and payment lifecycle | Guest requests; host approves; guest submits payment proof; host confirms; guest or ops opens dispute when needed; host or ops resolves it; guest reviews | Host declines with reason, invalid transition, guest direct completion, expired pending enquiry, expired approved hold, payment proof on wrong state, confirmation on wrong state, inaccessible stored proof asset, duplicate dispute open, dispute resolution without open case, availability conflict | Guest owns request/payment proof/dispute open/review; host owns approve/decline/confirm/dispute resolution for own listing; admin/support can inspect all bookings and resolve disputes | `inquiryExpiryCron` and `runInquiryExpiryCycle`; approved holds and confirmed stays update availability ledger; dispute resolution can push approved enquiries back to `FAILED` payment state | Booking status, payment requested, payment proof submitted, confirmation, decline, expiry notifications | Off-platform bank transfer plus private bucket-backed payment proof evidence; future payment provider classification | Strong unit/UI/client/e2e coverage: `tests/booking-workflow.test.ts`, `tests/inquiry-state.test.ts`, `tests/ui/guest-dashboard.test.tsx`, `tests/ui/host-dashboard.test.tsx`, client tests, `tests/e2e/happy-path.spec.ts`, `tests/e2e/booking-payment-review.spec.ts`, and contract coverage in `tests/payment-dispute-contract.test.ts` | Backend dispute permission/state-transition tests; Playwright dispute open/resolve flow; notification generation per transition |
