...
| Booking, enquiry, and payment lifecycle | Guest requests; host approves; guest submits payment proof; host confirms; guest reviews | Host declines with reason, invalid transition, guest direct completion, expired pending enquiry, expired approved hold, payment proof on wrong state, confirmation on wrong state, inaccessible stored proof asset, availability conflict | Guest owns request/payment proof/review; host owns approve/decline/confirm for own listing; admin can inspect all bookings | `inquiryExpiryCron` and `runInquiryExpiryCycle`; approved holds and confirmed stays update availability ledger | Booking status, payment requested, payment proof submitted, confirmation, decline, expiry notifications | Off-platform bank transfer plus private bucket-backed payment proof evidence; future payment provider classification | Strong unit/UI/client/e2e coverage: `tests/booking-workflow.test.ts`, `tests/inquiry-state.test.ts`, `tests/ui/guest-dashboard.test.tsx`, `tests/ui/host-dashboard.test.tsx`, client tests, `tests/e2e/happy-path.spec.ts`, `tests/e2e/booking-payment-review.spec.ts` | Backend expiry cycle with state mutation and availability release; notification generation per transition |
