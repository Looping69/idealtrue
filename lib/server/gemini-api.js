...
async function postJson(url, apiKey, payload) {
  ...
  try {
    ...
    throw new AiProviderError("gemini", "Gemini request failed before a response was received.", {
      retryable: true,
      cause: error,
    });
  }
  ...
  if (!response.ok) {
    throw new AiProviderError("gemini", parseGeminiError(body, response.statusText), {
...
export async function generateGeminiText({
  ...
  if (!text) {
    throw new AiProviderError("gemini", "Gemini returned an empty text response.", {
      retryable: true,
    });
  }
...
export async function generateGeminiImage({
  ...
  if (!inlineImage?.data || !inlineImage?.mimeType) {
    throw new AiProviderError("gemini", "Gemini did not return an image.", {
      retryable: true,
    });
  }
