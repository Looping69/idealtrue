...
const SOURCE_IMAGE_TIMEOUT_MS = 5000;
const ALLOWED_SOURCE_IMAGE_MIME_TYPES = new Set(["image/jpeg", "image/png", "image/webp", "image/gif"]);
...
function expandIpv6(value) {
  ...
  if (ipv4Match) {
    ...
    address = ipv4Match[1];
    ipv4Groups = [((bytes[0] << 8) | bytes[1]).toString(16), ((bytes[2] << 8) | bytes[3]).toString(16)];
  }
  ...
  const fill = new Array(8 - left.length - right.length - ipv4Groups.length).fill("0");
  const groups = halves.length === 2 ? [...left, ...fill, ...right, ...ipv4Groups] : [...left, ...right, ...ipv4Groups];
...
function isBlockedIpv6(value) {
  ...
  return (
    allZero ||
    groups[7] === 1 && groups.slice(0, 7).every((group) => group === 0) ||
...
async function assertResolvedImageHostIsPublic(parsed, imageDnsLookup = dnsLookup) {
  const hostname = stripIpv6Brackets(parsed.hostname.trim().toLowerCase());
  if (isIP(hostname)) return;

  const addresses = await imageDnsLookup(hostname, { all: true, verbatim: true });
  if (!Array.isArray(addresses) || addresses.length === 0) {
    throw new Error("Could not verify the selected image host.");
  }
  for (const result of addresses) {
    const address = stripIpv6Brackets(`${result.address || ""}`.trim().toLowerCase());
    if ((isIP(address) === 4 && isBlockedIpv4(address)) || (isIP(address) === 6 && isBlockedIpv6(address))) {
      throw new Error("The selected image host is not allowed.");
    }
  }
}
...
async function readResponseBodyWithLimit(response) {
  ...
  if (contentLength > MAX_SOURCE_IMAGE_BYTES) {
    throw new Error("The selected listing image is too large for template generation.");
  }
...
async function fetchImageAsDataUrl(url, { imageDnsLookup = dnsLookup } = {}) {
  const parsed = assertAllowedImageUrl(url);
  await assertResolvedImageHostIsPublic(parsed, imageDnsLookup);

  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), SOURCE_IMAGE_TIMEOUT_MS);
  let response;
  try {
  ...
  if (response.status >= 300 && response.status < 400) {
    throw new Error("Listing image redirects are not allowed for social creatives.");
  }
  if (!response.ok) {
  ...
  if (!ALLOWED_SOURCE_IMAGE_MIME_TYPES.has(mimeType)) {
    throw new Error("The selected listing image must be a PNG, JPEG, WebP, or GIF image.");
  }

  const dataBase64 = (await readResponseBodyWithLimit(response)).toString("base64");
...
export async function generateSocialTemplatePack({
  ...
  const imageDataUrls = await Promise.all(imageUrls.map((url) => fetchImageAsDataUrl(url, { imageDnsLookup })));
