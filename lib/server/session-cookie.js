...
export function allowsStagingEncoreBackend(env = process.env) {
  const rawValue = `${env.ALLOW_STAGING_ENCORE_BACKEND || ""}`.trim().toLowerCase();
...
export function resolveEncoreApiUrl(env = process.env, options = {}) {
  ...
  if (productionLike && hasStagingEncoreHost(normalizedUrl) && !allowsStagingEncoreBackend(env)) {
...
export function parseCookies(cookieHeader) {
  ...
  for (const part of cookieHeader.split(";")) {
    ...
    try {
      cookies[name] = decodeURIComponent(joinedValue);
    } catch {
      cookies[name] = joinedValue;
    }
  }
